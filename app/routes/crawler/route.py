...
def update_articles():
    try:
        # Ambil semua artikel yang belum ada embedding
        ...
        for article in articles:
            ...
            if cleaned_result:
                ...
                ideology_result = predictIdeology(cleaned_article['content'])
            ...
            processed_count += 1
            print("Processed article ID:", article.id)
            

        for item in update_data:
            article = item['article']
            if item['cluster'] is not None:
                article.cluster = item['cluster']
...
def group_articles():
    try:
        ...
        for idx in unique_title_indices:
            db.session.add(Title(title_index=idx))
        db.session.flush()  # pastikan baris title masuk dulu sebelum update articles
...
def process_articles():
    try:
        ...
        for record in title_records:
            ...
            # mode cluster
            mode_cluster = get_mode_cluster(formatted_articles)
            ...
            image_link = next((a.image.strip() for a in group_articles if a.image), None)
